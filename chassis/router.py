...
import time
from constellation.types import PacketEnvelope, TraceEntry, TerminalResult, ConstellationError, _uid
...
def route_packet(packet: PacketEnvelope, *, max_hops: int = 20, timeout_ms: float = 60000):
    ...
    for _ in range(max_hops):
        ...
        append_trace(
            packet,
            TraceEntry(
                node=node_name, action=current_action, status="completed", latency_ms=hop_latency
            ),
        )
