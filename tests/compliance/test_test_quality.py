...
def test_no_weak_tests():
    ...
    assert not weak, f"Weak tests ({len(weak)}):\n" + "\n".join(
        f"  [{f.code}] {f.file}:{f.line}" for f in weak
    )
...
def test_no_broken_fixtures():
    ...
    assert not broken, f"Broken fixtures ({len(broken)}):\n" + "\n".join(
        f"  [{f.code}] {f.file}:{f.line}" for f in broken
    )
